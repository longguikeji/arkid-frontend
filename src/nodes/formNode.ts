...
interface IGenerateConfigParams {
  ...
  valuesIsEmpty?: boolean,
}

export class FormNode extends StateNode {
  ...
  public get config() {
    return {
      params: FormNode.paramsters,
      request: FormNode.requestMapping,
      response: FormNode.responseMapping
    }
  }
  ...
  public generateFormPage(args: IGenerateConfigParams): FormPage {
    ...
    if (schema.discriminator && schema.oneOf) {
      ...
      for (const refValue in schema.discriminator.mapping) {
        ...
        refForms[refValue] = {
          items: this.getItemsBySchema(refSchema, showReadOnly, propertyName)
        }
      }
    } else {
      const formState:FormState = {}
      formPageState.form = formState
      formState.items = this.getItemsBySchema(schema, showReadOnly)
    }
    return formPageState
  }

  public getItemsBySchema(schema:ISchema, showReadOnly:boolean, skipProp = '') {
    ...
    for (const prop in schema.properties) {
      ...
      const item = this.createItemByPropSchema(prop, propSchema, showReadOnly)
      if (item) tempItems[prop] = item
    }
    return tempItems
  }

  public createItemByPropSchema(prop:string, schema: ISchema, showReadOnly:boolean):FormItemState | null {
    ...
    if (schema.page) {
      ...
      itemState.items = this.getItemsBySchema(schema, showReadOnly)
      item = {
        type: 'FormObjectItem',
        label: schema.title,
        prop: prop,
      ...
      item = this.createItemByPropSchema(prop, objectSchema, showReadOnly)
    }
    return item
  }
